...
import { makeStripeInvoiceCreatedEvent } from '@/tests/factories/make-stripe-invoice-created-event'
...
describe('Stripe Webhook - invoice.created (e2e)', () => {
	beforeAll(async () => {
		await app.ready()
	})
	...
	it('should handle stripe invoice.created event', async () => {
		const plan = makePlan()
		...
		const event = makeStripeInvoiceCreatedEvent({
