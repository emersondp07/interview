...
import { makeStripeInvoicePaidEvent } from '@/tests/factories/make-stripe-invoice-paid-event'
...
describe('Stripe Webhook - price.created (e2e)', () => {
	...
	it('should handle stripe invoice.paid event', async () => {
		const plan = makePlan()
			...
			data: {
				...
				signature: {
					create: {
						...
						stripe_subscription_id: mSignature.stripeSubscriptionId,
						stripe_subscription_status: mSignature.stripeSubscriptionStatus,
					},
				},
			},
		})

		const mInvoice = makeInvoice()

		await prisma.invoice.create({
			data: {
				id: mInvoice.id.toString(),
				mounth: mInvoice.mounth,
				value: mInvoice.value,
				status: mInvoice.status,
				issueDate: mInvoice.issueDate,
				dueDate: mInvoice.dueDate,
				signature_id: mSignature.id.toString(),
				stripe_invoice_id: `in_${faker.string.alphanumeric(14)}`,
			},
		})

		const event = makeStripeInvoicePaidEvent({
